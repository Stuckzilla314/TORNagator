  ...
  const injectMarketValues = useCallback((wvInstance) => {
    ...
    const injectScript = `
      (() => {
        ...
        window._tornagator_has_api_key = ${!!apiKey};
    ...
    if (payload.type === 'fetch') {
      ...
      try {
        if (!url.startsWith('https://api.torn.com/')) {
            throw new Error('Unauthorized URL requested via bridge');
        }
        const finalUrl = url.replace('BRIDGE_API_KEY', apiKey);
          ...
          if (isItemMarket) {
            ...
            if (!window._tornagator_run_seller_scan) {
              ...
              window._tornagator_run_seller_scan = async () => {
                ...
                const hasApiKey = window._tornagator_has_api_key;
                const userData = window._tornagator_user_data || {};
                ...
                const processSeller = async (seller, rowEl) => {
                  try {
                    const data = await hostFetch('https://api.torn.com/user/' + seller.id + '?selections=profile,personalstats&key=BRIDGE_API_KEY');
